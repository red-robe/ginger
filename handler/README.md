...
- 可使用ginger-gen 的handler命令生成处理函数，如：`ginger-gen handler -f user -F SignIn -F SignUp`,该工具会自动生成处理器代码；


### 关于请求参数验证器
默认带参数的请求其handler方法都需要独立编写请求参数struct，并在字段tag处编写验证字段，如：
```
/*
登录处理
*/
type LogInForm struct {
    AdminName string `json:"admin" validate:"required"`
