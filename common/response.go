...
func ResponseOk(c *gin.Context, data interface{}) {
	Response(c, ResponseCodeOk, http.StatusOK, data, "")
}

// 客户端错误响应的封装 （http 400）
func ResponseClientError(c *gin.Context, errorContent string) {
	Response(c, ResponseCodeClientError, http.StatusBadRequest, nil, errorContent)
}

// 服务端错误响应的封装 （http 500）
func ResponseServerError(c *gin.Context, errorContent string) {
	Response(c, ResponseCodeServerError, http.StatusInternalServerError, nil, errorContent)
}

// 参数校验错误响应体封装
func ResponseInvalidParam(c *gin.Context, errorContent string) {
	Response(c, ResponseCodeInvalidParam, http.StatusBadRequest, nil, errorContent)
}

// 未鉴权访问
func ResponseUnAuthorized(c *gin.Context, errorContent string) {
	Response(c, ResponseCodeUnAuthorized, http.StatusUnauthorized, nil, errorContent)
}

// 访问方法不被允许
func ResponseMethodNotAllowed(c *gin.Context, errorContent string) {
	Response(c, ResponseCodeMethodNotAllowed, http.StatusMethodNotAllowed, nil, errorContent)
}

// 返回空结果或查询不到结果
func ResponseEmptyResult(c *gin.Context, errorContent string) {
	Response(c, ResponseCodeEmptyResult, http.StatusOK, nil, errorContent)
}

// handler error response
func ResponseHandlerError(c *gin.Context, errorContent string) {
	Response(c, ResponseCodeHandlerError, http.StatusInternalServerError, nil, errorContent)
}

// model error response
func ResponseModelError(c *gin.Context, errorContent string) {
	Response(c, ResponseCodeModelError, http.StatusInternalServerError, nil, errorContent)
}

// repository error response
func ResponseRepositoryError(c *gin.Context, errorContent string) {
	Response(c, ResponseCodeRepositoryError, http.StatusInternalServerError, nil, errorContent)
}

// cache error response
func ResponseCacheError(c *gin.Context, errorContent string) {
	Response(c, ResponseCodeCacheError, http.StatusInternalServerError, nil, errorContent)
}

// casbin api error response
func ResponseCasbinApiError(c *gin.Context, errorContent string) {
	Response(c, ResponseCodeCasbinError, http.StatusForbidden, nil, errorContent)
}

// auth middleware error response
func ResponseAuthMiddlerwareError(c *gin.Context, errorContent string) {
	c.Abort()
	Response(c, ResponseCodeAuthMiddlerwareError, http.StatusForbidden, nil, errorContent)
}

// cors middleware error response
func ResponseCorsMiddlewareError(c *gin.Context, errorContent string) {
	c.Abort()
	Response(c, ResponseCodeCorsMiddlerwareError, http.StatusForbidden, nil, errorContent)
}

// rbac middleware error response
func ResponseRbacMiddlewareError(c *gin.Context, errorContent string) {
	c.Abort()
	Response(c, ResponseCodeRbacMiddlerwareError, http.StatusForbidden, nil, errorContent)
}
